<?php
// Ngăn truy cập trực tiếp  vào bộ điều khiển
...
class Dangnhap extends CI_Controller {
    ...
    public function dangnhap(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->data['title']='HNSHOP - Đăng nhập tài khoản';
            $this->data['view']='dangnhap';
            $this->load->view('frontend/layout',$this->data);
        }     
    }
    ...
    public function dangky(){
        ...
        $this->form_validation->set_rules('phone', 'Số điện thoại', 'required|min_length[6]|numeric|is_unique[db_customer.phone]|max_length[10]');
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $newcoupon=array(
                ...
                'description' => 'Mã giảm giá 100.000vnđ tự động khi đăng ký thành công',
                'created' => $today,
                'orders' => 0,
                'trash' => 1,
                'status' => 1,
            ...
            $this->email->clear();
            $config['protocol']    = 'smtp';// giao thuc
            ...
            $this->email->to($email);
            $this->email->subject('Hệ thống Xuhao store - Quà thành viên mới');
            $this->email->message('Bạn đã trở thành thành viên mới của cửa hàng Xuhao store, Cửa hàng tặng bạn 1 mã giảm giá giảm 100.000 vnđ : '.$tempcoupon.' , Mã này có giá trị tới ngày '.$tempdatelimit.'
        ...
        $this->data['title']='Xuhao store - Đăng ký tài khoản';   
    ...
    public function forget_password(){
        ...
        if($this->form_validation->run() ==TRUE){
            ...
            $this->email->subject('Hệ thống Xuhao store - Lấy lại mật khẩu');
        ...
        $this->data['title']='Xuhao store - Quên mật khẩu';   
    ...
    public function reset_password_new($id){
        ...
        if($this->form_validation->run() ==TRUE){ 
           ...
           if($this->Mcustomer->customer_check_id_email($id, $email)!=FALSE){
            ...
            $this->data['title']='Xuhao store - Cập nhật mật khẩu mới';
